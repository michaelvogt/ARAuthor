...
class MarkerListScreen : ScopedActions(idMatcher(R.id.marker_list_layout)) {
    ...
    fun inList(action: List.() -> Actions) =
            List(R.id.marker_list).action()

    fun inMarkerInfo(action: UiActions.() -> Actions) =
            UiActions(idMatcher(R.id.toolbar_marker_info)).action()

    fun inInfoPrompt(action: UiActions.() -> Actions) =
            UiActions(idMatcher(R.id.material_target_prompt_view)).action()

    companion object {
        const val title = "Markers"
        const val itemCount = 31
        ...
        fun open() {
            ...
            onView(withText(R.string.actionbar_marker_list_title)).perform(click())
        }
    }
}